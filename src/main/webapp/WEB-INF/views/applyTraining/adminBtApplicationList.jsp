...
<%--    <link rel="stylesheet" tyep="text/css" href="<c:url value="/css/regist.css"/>">--%>
    ...
    <title>정석코딩 관리자 | 부트캠프 신청서 관리</title>
    <%--폰트어썸 라이브러리 불러오기--%>
...
<div id="adminContent">
    ...
    <main>
        <div id="memberInfo">
            <div id="infoTitleBox">
                <h1>
                    <i class="fa-solid fa-rocket" style="color: #353739;"></i>
                    &nbsp;&nbsp;부트캠프 신청서 관리</h1>
            </div>
                ...
                <table>
                    ...
                    <c:forEach var="btApplicationDto" items="${list}">
                        <tr class="applicationManageTr">
                            ...
                            <td class="applicationManageTd"><a class="subject_name" href="<c:url value='/adminManage/adminBtTraining/read${ah.sa.queryString}&stfmNo=${btApplicationDto.stfmNo}'/>">${btApplicationDto.title}</a></td>
...
<%--                            <td class="applicationManageTd">${btApplicationDto.approvalYN}</td>--%>
                            <td class="applicationManageTd">
                                <c:choose>
                                    <c:when test="${btApplicationDto.approvalYN eq '승인'}">
                                        <span style="color: #23a723;">${btApplicationDto.approvalYN}</span>
                                    </c:when>
                                    <c:otherwise>
                                        <span style="color: #f61717;">${btApplicationDto.approvalYN}</span>
                                    </c:otherwise>
                                </c:choose>
                            </td>
            ...
            <div id="infoNaviBox">
                ...
                <c:if test="${totalCnt!=null && totalCnt!=0}">
                    <c:if test="${ah.showPrev}">
                        <a href="<c:url value='/adminManage/adminBtTraining/list${ah.sa.getQueryString(ah.beginPage-1)}'/>">
                    ...
                    <c:if test="${ah.showNext}">
                        <a href="<c:url value='/adminManage/adminBtTraining/list${ah.sa.getQueryString(ah.endPage+1)}'/>">
