...
<div>
  ...
  <div id="infoDetailBox">
    ...
    <h2 style="text-align: center;">회원 정보 상세보기</h2>
  ...
  <div class="infoValueBox">${memberDto.phone}</div>
    <h4>등급</h4>
    <input type="text" class="infoModifyBox" readonly value="${memberDto.grade}">
  <h4>상태</h4>
  <input type="text" class="infoModifyBox" readonly value="${memberDto.status}">
  <h4>환불 계좌</h4>
  <input type="text" class="infoModifyBox" readonly value="${memberDto.acct}">
  <h4>가입일</h4>
  <div class="infoValueBox"><fmt:formatDate value="${memberDto.regDate}" pattern="yyyy-MM-dd" type="date"/></div>
  <h4>비고</h4>
  <input type="text" class="infoModifyBox" readonly value="${memberDto.etc}">

    <div style="margin-left: 300px;margin-top: 20px;">
    <button class="infoDetailBtn"  onclick="location.href='/memberManage/modify?page=${page}&mebrNo=${memberDto.mebrNo}'">수정</button>
    <button class="infoDetailBtn" onclick="location.href='/memberManage/list?page=${page}'">목록</button>
  </div>
</div>
